...
export async function GET(req: Request) {
  ...
  for (const base of BASE_URLS) {
    try {
      const url = `${base}/api/search/songs?query=${encodeURIComponent(query)}&page=1&limit=25`
      const json = await tryFetchJson(url, 7000)
      if (json?.success && Array.isArray(json?.data?.results)) {
        const results: ModernSong[] = json.data.results.map(transformSong).filter(Boolean)
        if (results.length) {
          return NextResponse.json({ success: true, data: { results } })
        }
      }
